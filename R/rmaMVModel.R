  ...
  if( is.null(pred1) && is.null(pred2)){
    ...
    if(measure == "COR") {
      rma_mvmodel <- rma.mv(transf.rtoz(dat[,yi],dat[,o_ni]), transf.rtoz(dat[,vi],dat[,o_ni]),
                          random=list(~1 | outcome_ID ,~1 | sample_ID ,~1 | treat_ID , ~1 |report_ID),
  ...
  if( !is.null(pred1) && !is.null(pred2)){
    ...
    if(pred1["type"]=="num" && pred2["type"]=="num"){
      ...
      if(measure == "COR") {
        ...
        rma_mvmodel <- rma.mv(yi=dat[,yi],vi=dat[,vi],mods=~mod1+mod2,
                              random=list(~1 | outcome_ID ,~1 | sample_ID ,~1 | treat_ID , ~1 |report_ID),
                              measure=measure,data=dat)
    ...
    if(pred1["type"]=="cat"){
      ...
      rma_mvmodel <- rma.mv(yi=dat[,yi],vi=dat[,vi],mods=mod,
                            random=list(~1 | outcome_ID ,~1 | sample_ID ,~1 | treat_ID , ~1 |report_ID),
                            measure=measure,data=dat)
      return(rma_mvmodel)
    }else{
